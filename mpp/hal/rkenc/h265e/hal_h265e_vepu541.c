...
static void vepu541_h265_set_l2_regs(H265eV541HalContext *ctx, H265eV541L2RegSet *regs)
{
    ...
    regs->atf_thd0.atf_thd0_i32 = 0;
    regs->atf_thd0.atf_thd1_i32 = 63;
    regs->atf_thd1.atf_thd0_i16 = 0;
    regs->atf_thd1.atf_thd1_i16 = 63;
    regs->atf_sad_thd0.atf_thd0_p64 = 0;
    regs->atf_sad_thd0.atf_thd1_p64 = 63;
    regs->atf_sad_thd1.atf_thd0_p32 = 0;
    regs->atf_sad_thd1.atf_thd1_p32 = 63;
    regs->atf_sad_wgt0.atf_thd0_p16 = 0;
    regs->atf_sad_wgt0.atf_thd1_p16 = 63;
...
vepu541_h265_set_enc_infos(H265eV541HalContext *ctx, HalEncTask *task)
{
    ...
    info->cnt = 0;
    mpp_set_codec_info_elem(info, ENC_INFO_WIDTH, ENC_INFO_FLAG_NUMBER, prep->width);
    mpp_set_codec_info_elem(info, ENC_INFO_HEIGHT, ENC_INFO_FLAG_NUMBER, prep->height);

    {
        m_data = 0;
        strncpy((char *)&m_data, (const char *)"HEVC", sizeof(m_data));
        mpp_set_codec_info_elem(info, ENC_INFO_FORMAT, ENC_INFO_FLAG_STRING, m_data); // 0: 264, 1:265
    }
    mpp_set_codec_info_elem(info, ENC_INFO_FPS_IN, ENC_INFO_FLAG_NUMBER,
                            rc->fps_in_num / rc->fps_in_denorm);
    mpp_set_codec_info_elem(info, ENC_INFO_FPS_OUT, ENC_INFO_FLAG_NUMBER,
                            rc->fps_out_num / rc->fps_out_denorm);
    {
        m_data = 0;
        mpp_get_rc_mode_codec_info_data(rc->rc_mode, &m_data);
        mpp_set_codec_info_elem(info, ENC_INFO_RC_MODE, ENC_INFO_FLAG_STRING, m_data);
    }
    mpp_set_codec_info_elem(info, ENC_INFO_BITRATE, ENC_INFO_FLAG_NUMBER, rc->bps_target);

    mpp_set_codec_info_elem(info, ENC_INFO_GOP_SIZE, ENC_INFO_FLAG_NUMBER, rc->gop);
    mpp_set_codec_info_elem(info, ENC_INFO_FPS_CALC, ENC_INFO_FLAG_NUMBER, task->frame_rate);
    {
        m_data = 0;
        ...
        mpp_set_codec_info_elem(info, ENC_INFO_PROFILE, ENC_INFO_FLAG_STRING, m_data);
    }
    (void)task;
